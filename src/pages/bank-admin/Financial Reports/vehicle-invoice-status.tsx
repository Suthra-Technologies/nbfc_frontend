...
export function VehicleInvoiceStatusReport() {

    const handlePrint = () => {
        import('jspdf').then(({ default: jsPDF }) => {
            const doc = new jsPDF();
            doc.setFontSize(18);
            doc.text("Vehicle Invoice Status Report", 14, 20);
            ...
            doc.setFontSize(14);
            doc.text("Invoice Status Summary:", 14, 55);
            
            doc.setFontSize(10);
            const tableHeader = "Vehicle No  | Client Name       | Invoice No | Status      ";
            doc.text(tableHeader, 14, 70);
            doc.text("-".repeat(80), 14, 75);
            doc.text("KA-01-1234  | Rahul Kumar       | INV-9901   | Pending     ", 14, 83);
            doc.text("MH-12-5678  | Sneha Patil       | INV-9922   | Paid        ", 14, 91);
            
            doc.text("-".repeat(80), 14, 101);
            doc.setFontSize(11);
            doc.text("Total Records: 2", 14, 111);
                ...
                <Card className="flex-1 border-[#e2e8f0] shadow-sm bg-white overflow-hidden rounded-md min-h-[500px]">
                    <div className="h-full w-full flex items-center justify-center text-[#94a3b8]">
                        <div className="text-center space-y-2">
                            <p className="text-sm font-medium italic">Select date filter and click Print button.</p>
                            <p className="text-xs opacity-70 italic">Generates the current status of all vehicle invoices as of the selected date.</p>
                        </div>
                    </div>
                </Card>
